...
class Supertrend(bt.Indicator):
    ...
    # CRITICAL: Force bar-by-bar processing for stateful algorithm
    _nextforce = True  # Forces next() to be called instead of batch once() mode
    ...
    def nextstart(self):
        """
        Called ONCE for the first valid bar (after minimum period).
        Initialize the Supertrend state.
        """
        # Get current values
        ...
        if close > basic_lower:
            self.direction[0] = 1
            self.supertrend[0] = basic_lower
        else:
            self.direction[0] = -1
            self.supertrend[0] = basic_upper
    ...
    def next(self):
        ...
        # Smooth the bands using previous values
        ...
        # Determine direction using previous direction
