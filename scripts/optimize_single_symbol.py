"""
Optimize Supertrend parameters for a single symbol
Generalized version that works with any CSV file
"""
...
def generate_parameter_grid():
    ...
    param_grid = {
        ...
        'stop_loss_value': [None, -0.10, -0.15],
        'profit_target': [None, 0.50, 1.00, 2.00],
    }
...
def optimize_symbol(symbol, csv_file, top_n=10):
    """
    ...
    """
    print("="*80)
    print(f"{symbol} SUPERTREND PARAMETER OPTIMIZATION")
    print("="*80)
    print()
    ...
    print(f"Price: ${df['close'].iloc[0]:.2f} → ${df['close'].iloc[-1]:.2f} ({bh_return:,.1f}% buy-and-hold)")
    print()
    ...
    # Display top results
    print("\n" + "="*80)
    print(f"TOP {top_n} CONFIGURATIONS (by Return)")
    print("="*80)
    print()

    for idx, (i, row) in enumerate(results_df.head(top_n).iterrows(), 1):
        print(f"\n#{idx} - Return: {row['return']:.2f}%")
    ...
    # Save results
    output_file = f'data/results/{symbol}_optimization_results.csv'
    os.makedirs(os.path.dirname(output_file), exist_ok=True)
    results_df.to_csv(output_file, index=False)
    print(f"\n✅ Full results saved to {output_file}")

    # Return top result for comparison
    top_result = results_df.iloc[0].to_dict()
    top_result['symbol'] = symbol
    top_result['buy_hold_return'] = bh_return
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 3:
        print("Usage: python optimize_single_symbol.py SYMBOL CSV_FILE")
        print("Example: python optimize_single_symbol.py AMD data/raw/AMD_daily.csv")
        sys.exit(1)

    symbol = sys.argv[1]
    csv_file = sys.argv[2]

    optimize_symbol(symbol, csv_file)
